...
func TestSimulatedBroker_ConnectAndDisconnect(t *testing.T) {
	b := NewSimulatedBroker(1_000_000)
	require.NoError(t, b.Connect())
	require.NoError(t, b.Disconnect())
}

func TestSimulatedBroker_SubmitMarketOrder_FillsImmediately(t *testing.T) {
	b := NewSimulatedBroker(1_000_000)
	require.NoError(t, b.Connect())

	id, err := b.SubmitOrder(domain.Order{
		Symbol:    "X",
	...
	status, _ := b.GetOrderStatus(id)
	assert.Equal(t, "filled", status)

	balance, _ := b.GetAccountBalance()
	assert.Less(t, balance, 1_000_000.0, "cash must be debited")
}

func TestSimulatedBroker_SubmitWithoutConnect(t *testing.T) {
	b := NewSimulatedBroker(1_000_000)
	_, err := b.SubmitOrder(domain.Order{Symbol: "X", Direction: domain.DirectionLong, OrderType: domain.OrderTypeMarket, Quantity: 1})
	require.Error(t, err)
}

func TestSimulatedBroker_CancelUnknown(t *testing.T) {
	b := NewSimulatedBroker(1000)
	require.NoError(t, b.Connect())
	err := b.CancelOrder("nope")
	require.Error(t, err)
}

func TestSimulatedBroker_CancelFilled(t *testing.T) {
	b := NewSimulatedBroker(1_000_000)
	require.NoError(t, b.Connect())
	id, err := b.SubmitOrder(domain.Order{
		Symbol: "X", Direction: domain.DirectionLong, OrderType: domain.OrderTypeMarket, Quantity: 1, LimitPrice: 50,
	})
	require.NoError(t, err)

	err = b.CancelOrder(id)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "cannot cancel filled")
}

func TestSimulatedBroker_GetPositionsAndBalance(t *testing.T) {
	b := NewSimulatedBroker(100_000)
	_, _ = b.GetPositions()
	bal, err := b.GetAccountBalance()
	require.NoError(t, err)
	assert.Equal(t, 100_000.0, bal)
}

func TestSimulatedBroker_SetPrice(t *testing.T) {
	b := NewSimulatedBroker(1_000_000)
	require.NoError(t, b.Connect())
	_, err := b.SubmitOrder(domain.Order{
		Symbol: "X", Direction: domain.DirectionLong, OrderType: domain.OrderTypeMarket, Quantity: 10, LimitPrice: 50,
	})
	require.NoError(t, err)
	...
	assert.Equal(t, 75.0, positions[0].CurrentPrice)
	// MarketValue / UnrealizedPnL use the LimitPrice basis + slippage, not
	// the post-SetPrice price — verify SetPrice took effect on CurrentPrice only.
	assert.Equal(t, 10.0, positions[0].Quantity)
	assert.Greater(t, positions[0].MarketValue, 0.0)
	assert.NotZero(t, positions[0].UnrealizedPnL)

	// SetPrice on unknown symbol is a silent no-op
	b.SetPrice("NOPE", 1)
}
...
// ---- AdvancedMockTrader (no OrderStore path) -----------------------------

func TestAdvancedMockTrader_ConstructorDefaults(t *testing.T) {
	logger := newTestLogger()
	trader := NewAdvancedMockTrader(PersistentMockTraderConfig{}, logger)
	require.NotNil(t, trader)
	assert.Equal(t, "persistent_mock_trader", trader.Name())
	assert.NoError(t, trader.HealthCheck(context.Background()))
}

func TestAdvancedMockTrader_Lifecycle(t *testing.T) {
	trader := NewAdvancedMockTrader(PersistentMockTraderConfig{}, newTestLogger())
	require.NoError(t, trader.Connect(context.Background()))
	assert.True(t, trader.GetConnectionStatus().Connected)
	require.NoError(t, trader.Disconnect())
}

func TestAdvancedMockTrader_TradesAndCashFlow(t *testing.T) {
	trader := NewAdvancedMockTrader(PersistentMockTraderConfig{}, newTestLogger()).(*advancedMockTrader)
	now := time.Now()
	trader.trades = []TradeRecord{
		{TradeID: "T1", OrderID: "O1", TradeTime: now},
		{TradeID: "T2", OrderID: "O1", TradeTime: now.Add(-24 * time.Hour)},
	}
	trader.cashFlows = []CashFlow{
		{ID: 1, Type: "deposit", Amount: 1000, Balance: 1000, CreatedAt: now},
		{ID: 2, Type: "withdraw", Amount: 500, Balance: 500, CreatedAt: now.Add(-1 * time.Hour)},
	}

	got, err := trader.GetTrades(context.Background(), "O1")
	require.NoError(t, err)
	assert.Len(t, got, 2)

	today, err := trader.ListTodayTrades(context.Background())
	require.NoError(t, err)
	assert.Len(t, today, 1, "only T1 is from today")

	cf, err := trader.GetCashFlow(context.Background(), nil, nil)
	require.NoError(t, err)
	assert.Len(t, cf, 2)
	...
	cf2, err := trader.GetCashFlow(context.Background(), &start, &end)
	require.NoError(t, err)
	assert.Len(t, cf2, 1, "only the recent deposit falls in the window")
}

func TestAdvancedMockTrader_UnsubscribeQuotes(t *testing.T) {
	trader := NewAdvancedMockTrader(PersistentMockTraderConfig{}, newTestLogger())
	require.NoError(t, trader.UnsubscribeQuotes([]string{"X"}))
}

func TestAdvancedMockTrader_SubscribeQuotes_ReceivesData(t *testing.T) {
	trader := NewAdvancedMockTrader(PersistentMockTraderConfig{}, newTestLogger())
	ch, err := trader.SubscribeQuotes(context.Background(), []string{"X"})
	require.NoError(t, err)

	select {
	case <-ch:
		// ok — at least one quote arrived within 1.5s
	case <-time.After(2 * time.Second):
		t.Fatal("no quote received within 2s")
	}
}

func TestAdvancedMockTrader_GetPosition_NotFound(t *testing.T) {
	trader := NewAdvancedMockTrader(PersistentMockTraderConfig{}, newTestLogger())
	_, err := trader.GetPosition(context.Background(), "X")
	require.Error(t, err)
}

func TestAdvancedMockTrader_GetPosition_Found_WithPnL(t *testing.T) {
	trader := NewAdvancedMockTrader(PersistentMockTraderConfig{
		MockTraderConfig: MockTraderConfig{
			InitialCash: 100_000,
			PriceProvider: func(string) float64 { return 12 },
		},
	}, newTestLogger()).(*advancedMockTrader)
	trader.positions["X"] = &PositionInfo{
		Symbol: "X", Quantity: 100, AvgCost: 10, AvailableQty: 50, CurrentPrice: 12, MarketValue: 1200, UnrealizedPnL: 200,
	}
	detail, err := trader.GetPosition(context.Background(), "X")
	require.NoError(t, err)
	require.NotNil(t, detail)
	assert.Equal(t, "X", detail.Symbol)
	assert.Greater(t, detail.ProfitRatio, 0.0, "profitable position must yield positive ratio")
}
