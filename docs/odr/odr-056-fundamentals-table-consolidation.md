...
> **Related ADRs**: [ADR-022](../adr/adr-022-unified-research-platform.md)（§2 按数据性质分区：同一性质的数据只应有一处权威存储 —— `fundamentals` 与 `stock_fundamentals` 同属类 C 派生数据，重叠违反分区唯一性）
> **Supersedes**: —
...
- **单一事实源**：12 个指标列在库中只有一处权威存储，符合 [ADR-022](../adr/adr-022-unified-research-platform.md) §2「按数据性质分区」的唯一性要求。
...
| **修改** | `docs/RESEARCH.md` | §3.7 C-8 行标注收口 |
...
| **新建** | `docs/odr/odr-056-fundamentals-table-consolidation.md` | 本文件 |
