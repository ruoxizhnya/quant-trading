...
> **Date**: 2026-09-15
> **Category**: Refactor
> **Related ADRs**: [ADR-022](../adr/adr-022-unified-research-platform.md)（本操作的架构决策）, [ADR-021](../adr/adr-021-equitydeep-research-layer.md)（被取代）
...
1. **起点**：[ODR-047](odr-047-equitydeep-integration-audit.md) + [ADR-021](../adr/adr-021-equitydeep-research-layer.md) 完成了 EquityDeep 与 Quant Lab 的定位复核，结论为「横截面层与纵向层正交 → **补充非改变**」，并决定"两仓独立、只锁数据契约"。
...
| ADR-021 既有引用大面积失效 | 低 | ADR-021 保留并标记 Superseded；`RESEARCH.md` 降级为"工作面 1 详案"并加 ADR-022 待修订说明块（已完成） |
...
| **新建** | `docs/adr/adr-022-unified-research-platform.md` — 架构决策 |
| **新建** | `docs/odr/odr-048-top-level-product-redefinition.md` — 本文件 |
| **修改** | `docs/adr/adr-021-equitydeep-research-layer.md` — Status → Superseded by ADR-022 + 取代说明 |
| **修改** | `docs/ADR.md` — 新增 ADR-022 / ODR-048 索引行；ADR-021 状态更新；版本 3.4.0 → 3.5.0；变更日志 |
| **修改** | `AGENTS.md` — 产品定位 / 文档导航 / 当前状态 / 已知问题 |
| **修改** | `docs/RESEARCH.md` — 降级为"工作面 1 详案"（v0.2.0）；头部加 ADR-022 待修订说明块（6 项），§1/§2/§3 有效内容保留，顶层定位改由 PRODUCT.md 承担 |
...
| 修改文档 | 10（ADR-021 / ADR.md / AGENTS.md / RESEARCH.md / TASKS.md / PRODUCT Spec / TECHNICAL Spec / VISION.md / ARCHITECTURE.md + 本文件） |
