...
verified-by: AUD-44 路径引用复核（2026-09-22）—— `pkg/tracker` 已迁至 `pkg/backtest/tracker`、`docs/phase-gate-reviews.md` 已归档至 `docs/archive/research-2026-Q2/`、§4 的 CLI 示例标注为非真实接口（analysis-service 是 HTTP 服务，没有这些 flag）；**§5/§6/§7 的判据未逐条复核**（见 TASKS.md AUD-47）
---
...
|------|----------|-------------|
| `src/utils/format.test.ts` | 8 tests | fmtPercent, fmtNumber, fmtCurrency, formatDate, fmtVolume, fmtMetric |
...
|-------|-------|-------------|
| Backtest Engine — Core | 6 | Basic rendering, form submission, duplicate prevention |
| Complete Workflow Tests | 12 | Run→Results→Trades→History→Report→Clear |
| Error Handling & Edge Cases | 4 | Loading state, invalid input, state persistence |
