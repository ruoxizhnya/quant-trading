...
**AUD 线至此 AUD-01 ~ AUD-57 全部有裁决且已落地。⛔ 未完成的是 AUD-58 / AUD-59 / AUD-60 三条**
（都是 2026-09-25 **跑起来才撞出来的**，都不是审查登记项）：
**AUD-58** = e2e 自身速率打爆网关限流（机制现成但没人接线，两条路待裁决）；
**AUD-59** = 日历同步的**作业路径**声称收两种日期格式、执行器只认 `YYYYMMDD`，
传 `YYYY-MM-DD` 会**静默**落进错误区间（校验器的承诺 ⊋ 消费者的能力）；
**AUD-60** = 回测的**输入/数据前置**失败被统一报成 `500 internal`（`codeForStatus` 是
「状态码→错误码」单向派生，于是 `INVALID_INPUT` 被改写成 `internal`，还把可自解的问题
记成服务端错误）。三条都只改了**登记**，代码一行未动 —— 动之前先裁决。
